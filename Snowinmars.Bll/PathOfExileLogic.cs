...
namespace Snowinmars.Bll
{
	public class PathOfExileLogic
	{
		...
		public IDictionary<int, IList<IList<int>>> PickQualityCombination(IList<int> qualities, int desiredValue)
		{
			...
			var (incompleteQualities, completeQualitiesCount) = ExtractMaxQualities(qualities);
		...
		//	for (int i = 0; i < completeQualitiesCount; i++)
		//	{
		//		combinations.Add(new List<int> { 20 });
		//	}
		...
		//	for (var i = 0; i < ungroupedCombinations.Count; i++)
		//	{
		//		var ungroupedCombination = ungroupedCombinations[i];

		//		IList<IList<int>> fdsgdf = ungroupedCombinations;

		//		foreach (var quality in ungroupedCombination)
		//		{
		//			fdsgdf = fdsgdf.Where(u => !u.Contains(quality)).ToList();
		//		}

		//		if (fdsgdf.Any())
		//		{
		//			PathOfExileLogic.Addd(ungroupedCombination, group, result);

		//			for (var j = 0; j < fdsgdf.Count; j++)
		//			{
		//				var item = fdsgdf[j];
		//				PathOfExileLogic.Addd(item, group, result);
		//				//ungroupedCombinations.Remove(item);
		//			}
		...
		//		else
		//		{
		//			PathOfExileLogic.Addd(ungroupedCombination, 0, result);
		//		}
		//	}

		//	for (int i = 0; i < completeQualitiesCount; i++)
		//	{
		//		PathOfExileLogic.Addd(new List<int> { 20 }, 0, result);
		//	}

		//	return result;
		//}

		private static void Addd(IList<int> ungroupedCombination, int group, IDictionary<int, IList<IList<int>>> result)
		{
			if (result.ContainsKey(group))
			{
				result[group].Add(ungroupedCombination);
			}
			else
			{
				result.Add(group, new List<IList<int>> { ungroupedCombination });
			}
		}
